...
class ViewController: UIViewController {
    ...
    func setWord() {
        ...
        letterLabel.text = "This word has " + String(currentWord.count) + " letters"
    }
...
//        confettiView.startAnimating()
    ...
    @IBAction func guessTapped(_sender: Any) {
        ...
        numGuesses += 1
        if (guessTextField.text == currentWord) {
            guessedCorrect()
        }
        else {
            guessedWrong()
    ...
    @IBAction func hintTapped(_sender: Any) {
        ...
        let hint = hints[currentWordIndex]
        let hintAlert = UIAlertController(title: "This is a hint", message: hint, preferredStyle: UIAlertController.Style.alert)
        hintAlert.addAction(UIAlertAction(title: "OK", style: .default, handler: nil))
        self.present(hintAlert, animated: true)
    }
